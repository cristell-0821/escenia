...
async function aprobarInscripcion(formData: FormData) {
  'use server'
  ...
  revalidatePath(`/admin/concursos/${contestId}/inscripciones`)
}

async function rechazarInscripcion(formData: FormData) {
  'use server'
...
export default async function InscripcionesPage({ params }: Props) {
  ...
  const { data: concurso } = await supabase
    .from('contests')
    .select('id, title, type, requires_approval')
    .eq('id', id)
    .single()
      ...
      <div className="bg-[#85332a] text-white px-8 py-6">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-white/60 mt-2">
            {concurso.type === 'official' ? 'Concurso Oficial' : 'Concurso Privado'}
                ...
                <div key={inscripcion.id} className="p-6 hover:bg-[#faf3e7] transition-colors">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-4">
                      {/* Logo o placeholder 
                      <div className="w-16 h-16 bg-[#eee7db] flex items-center justify-center text-2xl">
                        {inscripcion.group?.logo_url ? (
                          <img 
                            src={inscripcion.group.logo_url} 
                            alt="" 
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {inscripcion.status === 'pending' && (
                        <div className="flex gap-2 mt-2">
                          <form action={aprobarInscripcion}>
                          ...
                          <form action={rechazarInscripcion}>
                            <input type="hidden" name="id" value={inscripcion.id} />
                            <input type="hidden" name="contestId" value={id} />
                            <button
                              type="submit"
